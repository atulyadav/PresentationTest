...
$.fn.currPanelNumber = function(e) {
alert("currPanelNumber = "+panelnum+" = "+e);
};
$.fn.swipeToPanelNumber = function(e) {
var num = parseInt(e, 10);
    panelnum=num;
//    alert("swipeToPanelNumber =----- "+panelnum+"  "+num);
    if(num==1)
        panelswipeTo(0,num);
    else if(num==2)
        panelswipeTo(-1024,num);
    else if(num==3)
        panelswipeTo(-2053,num);
};
//var navig=null
//$.fn.panelswipePrev = function() {
////alert("preev cl");
//panelnum=3;
//panelswipeTo(-2053);
//};
//$.fn.panelswipeNext = function() {
////alert("next panel");
//panelnum=1;
//panelswipeTo(0);
//};
//$.fn.NavigateOnSwipeNext = function(){
//navig="navRight";
//NavigateOnSwipe();
//}
//$.fn.NavigateOnSwipePrev = function(){
//navig="navLeft";
//NavigateOnSwipe();
//}
...
//swipe to the first and last panel
var panelswipeTo=function(tem,_panelnum) {
//alert("panelswipeTo function panelnum = "+_panelnum+" tem = "  +tem);
   if (o.HORIZ==true) { len = vw; a = 'left'; } else { a='top'; len = vh; }
  //panelnum = Math.ceil(Math.abs((tem/panel.length) / (len/panel.length)));
//  alert("panelswipeTo function = "+panelnum+" tem = "  +tem);
//  if (_lastMouseDownXY-_mouseDownXY <= -(o.SNAPDISTANCE)) {
//      if (panelnum >= panel.length) { panelnum=panel.length-1; }
//      animate(a,-len * panelnum,len);
//  } else if (_lastMouseDownXY-_mouseDownXY >= o.SNAPDISTANCE) {
...
//  }
panelnum=_panelnum;
//alert("panelswipeTo function panelnum = "+panelnum+" tem = "  +tem);
  animate(a,(panelnum * -len) + len,len);
//  alert("next panel");
};
...
////to navigate on swipe
//var NavigateOnSwipe = function() {
//  if (navig=='navTop') {len = vh; a = 'top'; b = -sliderH + len; d = parseInt(plugin.css(a)) + len; }
//    else if (navig=='navBottom') {len = vh; a = 'top'; b = -sliderH + len; d = parseInt(plugin.css(a)) - len; }
//    else if (navig=='navLeft') {len = vw; a = 'left'; b = -sliderW + len; d = parseInt(plugin.css(a)) + len; }
//    else {len = vw; a = 'left'; b = -sliderW + len; d = parseInt(plugin.css(a)) - len; } //navRight
//    if (panelnum>panel.length) { panelnum=1; animate(a,0,len); return; }
//    if (panelnum<1) { panelnum=panel.length; animate(a,b,len); return; }
//    animate(a,d,len); //panel by panel
